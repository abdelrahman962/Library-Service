...
from cache import cached_call, invalidate_all


# NOT a single shared client: a module-level `api = LibraryAPIClient()`
# here used to mean every tool call, from every browser session on a
# Streamlit server, shared one mutable client/token — logging in on
# one tab silently reauthenticated every other tab's next tool call as
# the new member (confirmed: it surfaced as one member being told
# another member's real name). A contextvar keyed per (caller-bound)
# context fixes this because LangGraph explicitly propagates the
# calling thread's context into tool execution even though tools run
# on a different worker thread (confirmed empirically — tool calls
# see the context set before agent.invoke(), despite running
# elsewhere) — so each Streamlit session, which calls set_api() with
# its own st.session_state-held client at the top of every script run,
# gets its own client for every tool call made during that run.
_current_api: contextvars.ContextVar[LibraryAPIClient | None] = contextvars.ContextVar(
    "current_api", default=None
)


def set_api(client: LibraryAPIClient) -> None:
    """Bind the API client tool calls should use for the current context (thread)."""
    _current_api.set(client)


def get_api() -> LibraryAPIClient:
    """
    Return the API client bound for the current context. Every tool
    below calls this instead of touching a shared global, so it always
    gets whichever session's client is actually current.
    """
    client = _current_api.get()
    if client is None:
        raise RuntimeError(
            "No API client bound for this context — call set_api(client) "
            "once at the top of each session's script run before any "
            "tool can be invoked."
        )
    return client
...
def search_books(search: str) -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("search_books", {"search": search}, lambda: get_api().search_books(search))
...
def list_available_books() -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("list_available_books", {}, get_api().available_books)
...
def list_borrowed_books() -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("list_borrowed_books", {}, get_api().borrowed_books)
...
def list_all_books() -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("list_all_books", {}, get_api().all_books)
...
def get_book_details(book_id: int) -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("get_book_details", {"book_id": book_id}, lambda: get_api().get_book(book_id))
...
def my_profile() -> dict[str, Any]:
    ...
    """
    try:
        # Keyed partly on the current client's token — see my_borrowed_books below for
        # why (this is per-authenticated-member data on shared cache
        # storage).
        return cached_call("my_profile", {"token": get_api().token}, get_api().get_current_user)
...
def my_borrowed_books() -> dict[str, Any]:
    ...
    """
    try:
        # Keyed partly on the current client's token, not just the tool name: this
        # result is specific to whoever is currently authenticated,
        # and the cache is shared Redis storage — without this, a
        # second session logged in as a different member could be
        # served the first member's cached borrowed-books list.
        return cached_call("my_borrowed_books", {"token": get_api().token}, get_api().my_borrowed_books)
...
def my_borrow_history() -> dict[str, Any]:
    ...
    """
    try:
        # See my_borrowed_books above — keyed partly on the current client's token so
        # two sessions authenticated as different members never share
        # each other's cached history.
        return cached_call("my_borrow_history", {"token": get_api().token}, get_api().my_borrow_history)
...
def borrow_book(book: str) -> dict[str, Any]:
    ...
    the one asked for — see member_borrow_history/book_borrow_history
    ...
    try:
        ...
        result = get_api().borrow_book(book_id)
        if result.get("success"):
            # A borrow changes book availability, stats, this
            # member's borrowed-books list, and borrow-history
            # records — invalidate everything cached rather than
            # tracking exactly which keys that touches.
            invalidate_all()
        return result
...
def return_book(book: str) -> dict[str, Any]:
    ...
    try:
        book_id = _resolve_book_id(book)
        result = get_api().return_book(book_id)
        if result.get("success"):
            # See borrow_book above.
            invalidate_all()
        return result
...
def list_members(search: str = "") -> dict[str, Any]:
    ...
    def fetch() -> dict[str, Any]:
        ...
        while page <= last_page and page <= 20:
            result = get_api().list_members(search or None, page)
    ...
    try:
        return cached_call("list_members", {"search": search}, fetch)
...
def _resolve_member_id(member: str) -> int:
    ...
    result = get_api().list_members(member, 1)
...
def _resolve_book_id(book: str) -> int:
    ...
    result = get_api().search_books(book)
...
def book_borrow_history(book: str) -> dict[str, Any]:
    ...
    def fetch() -> dict[str, Any]:
        book_id = _resolve_book_id(book)
        result = get_api().book_history(book_id)
        return {
            "success": True,
    ...
    try:
        return cached_call("book_borrow_history", {"book": book}, fetch)
    except LibraryAPIError as e:
        return {
            "success": False,
            "message": str(e),
...
def member_borrow_history(member: str) -> dict[str, Any]:
    ...
    def fetch() -> dict[str, Any]:
        ...
        result = get_api().member_history(member_id)
        return {
            "success": True,
    ...
    try:
        return cached_call("member_borrow_history", {"member": member}, fetch)
    except LibraryAPIError as e:
        return {
            "success": False,
            "message": str(e),
...
def library_stats() -> dict[str, Any]:
    ...
    """
    try:
        return cached_call("library_stats", {}, get_api().library_stats)
